...
class ApplicationStatusRepository {
  private model: typeof ApplicationStatus;

  constructor(sequelize: Sequelize) {
    this.model = sequelize.models.ApplicationStatus as typeof ApplicationStatus;
  }
  ...
  async findById(id: number): Promise<ApplicationStatus | null> {
    try {
  ...
  async softDelete(id: number): Promise<ApplicationStatus | null> {
    try {
      const existingDocument = await this.model.findByPk(id);
